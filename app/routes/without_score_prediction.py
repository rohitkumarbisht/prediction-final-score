...
class WithoutScorePrediction(FlaskView):
    ...
    def insert_data_to_database(self, new_df, prediction):
        ...
        try:
            ...
            cursor.close()
            conn.close()
            return "values inserted successfully into database"
        except Exception as e:
            print(f"Error inserting data into PostgreSQL table: {str(e)}")
            return f"Error inserting data into PostgreSQL table:{str(e)}", 500
    ...
    def post(self):
        try:
            model_pkl = open_model('reg_model_without_score.pkl', 'rb')
