...
class EngagementPrediction(FlaskView):
    ...
    def get_final_score_columns(self):
        ...
        without_score_column = read_file(
            "highly_correlated_columns_without_score.txt", "r")
        with_score_column = read_file(
            "highly_correlated_columns_with_score.txt", "r")
        without_score = [
            item for item in without_score_column if item not in engagement_columns]
        with_score = [
            item for item in with_score_column if item not in engagement_columns]
